...
# import casadi as ca
import numpy as np
from husky_assembly_tamp.utils.params import DATA_DIR, PICK_DIRECTION, PROJECT_DIR
...
from husky_assembly_tamp.utils.util import calculate_pose_error, normalize_angles
...
if not os.path.isfile(HUSKY_DUAL_URDF_PATH):
    raise FileNotFoundError(f"HUSKY_DUAL_URDF_PATH does not exist: {HUSKY_DUAL_URDF_PATH}")

HUSKY_DUAL_SRDF_PATH = os.path.join(DATA_DIR, "husky_urdf/mt_husky_dual_ur5_e_moveit_config/config/dual_arm_husky.srdf")
if not os.path.isfile(HUSKY_DUAL_SRDF_PATH):
    raise FileNotFoundError(f"HUSKY_DUAL_SRDF_PATH does not exist: {HUSKY_DUAL_SRDF_PATH}")
