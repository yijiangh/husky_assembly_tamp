...
# ---------------------------------------------------------------------------
# ssik sidecar path resolution
# ---------------------------------------------------------------------------
# The ssik venv (Python 3.11 + ssik installed) and the built per-arm solver
# modules are machine-local resources that live in the HOST repo -- the repo that
# vendors this package as ``external/husky_assembly_tamp`` (the design repo has
# them at ``external/ssik_env`` and ``asset/ssik``). The code moved here but those
# resources did not, so each path is resolved at SPAWN time (never at import
# time) with the order: environment variable -> host-repo default -> clear error.
...
def get_ssik_artifact_dir() -> str:
    ...
    Resolution order: ``HUSKY_SSIK_ARTIFACT_DIR`` env var -> ``<host>/asset/ssik``
    -> error.

    Returns:
        str: the artifact folder path (existence is checked by the spawner).

    Raises:
        RuntimeError: when no folder can be determined (env var unset and this
            checkout is not vendored inside a host repo).
    """
    ...
    if host is None:
        raise RuntimeError(
            "Cannot locate the ssik artifacts: HUSKY_SSIK_ARTIFACT_DIR is not set and this "
            f"husky_assembly_tamp checkout ({_TAMP_REPO_ROOT}) is not vendored under a "
            "host repo's external/ folder. Run the one-time `ssik build` for both arms "
            "(see keyframe/ssik_sidecar/README.md) and set HUSKY_SSIK_ARTIFACT_DIR to the "
            "output folder, or set HUSKY_IK_BACKEND=gradient."
        )
    return os.path.join(host, "asset", "ssik")
