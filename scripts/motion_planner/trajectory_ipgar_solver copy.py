...
class TrajectoryIPGARSolver:
        ...
        for iteration, density in enumerate(density_levels):
            ...
            while len([item for sublist in sorted_obstacles for item in sublist]) != 0:
                ...
                for sub_obs_list in sorted_obstacles:
                    ...
                    # 如果找到碰撞，添加该元素中所有SDF值小于阈值的障碍物
                    if find_collision:
                        for name, obs, sdf_val in sub_obs_list[:]:
                            if sdf_val <= self.sdf_threshold:
                                del remaining_obstacles_dict[name]
                                active_obstacles.append(obs)
                                obs_id = pp.create_sphere(obs["radius"])
                                ...
                                active_obstacles_body.append(obs_id)
                                current_obs_ids.append(obs_id)
                                print(f"    IPGAR: Activated additional obstacle {name}")
                                sub_obs_list.remove((name, obs, sdf_val))
    ...
    def _repair_path(self, path, collision_intervals, collision_fn, active_obstacles_body, active_obstacles, pose_2d):
        ...
        def get_sample_fn(lower, upper):
            ...
            frozen_joints = self.robot_setup.base_joints
            frozen_values = [0, 0, 0]
        ...
        while collision_intervals:
            ...
            while repair_attempts < max_repair_attempts:
                ...
                # 使用扩展后的限制创建采样函数
                sample_fn = get_sample_fn(lower=np.array([-5.0, 5.0, -np.pi] + [limit[0] for limit in expanded_limits]), upper=np.array([5.0, 5.0, np.pi] + [limit[1] for limit in expanded_limits]))

                with pp.LockRenderer():
                    repair_result = self.robot_setup.plan_manipulator_path(q_start_local, q_end_local, self.robot_setup.attachments, active_obstacles_body, max_time=max_plan_time, max_iterations=10000, sample_fn=sample_fn)
