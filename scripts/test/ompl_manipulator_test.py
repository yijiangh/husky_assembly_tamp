...
class TrajectoryOMPLSolver:
    def __init__(
        ...
        self.space.setLongestValidSegmentFraction(1.0 / 180 * np.pi)
        ...
        self.planner.setRange(1.0 / 180 * np.pi)
    ...
    def plan(
        self, start_angles: np.ndarray, goal_angles: np.ndarray, interp_num: int = 100, time: float = 10.0
    ) -> np.ndarray:
        """
        ...
        if solved:
            ...
            # path.interpolate(num_states_raw * interp_num)
            # num_states = num_states_raw * interp_num
