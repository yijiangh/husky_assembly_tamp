...
config_file = load_yaml(join_path(get_robot_path(), "husky_ur5_e.yml"))
...
q = torch.tensor([[-1.299, -0.109, -1.384, 1.493, 1.843, -1.571]], **(tensor_args.as_torch_dict()))

out = kin_model.get_state(q)

print("joint names: ", robot_cfg.cspace.joint_names)

print("ee position: ", out.ee_pose.position.cpu().numpy().flatten().tolist())
print("ee quat: ", out.ee_pose.quaternion.cpu().numpy().flatten().tolist())
